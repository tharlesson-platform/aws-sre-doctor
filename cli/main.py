...
def _load_serialized_file(path: Path) -> dict:
...
def export_github_issue(
    report_path: Path = typer.Option(..., exists=True, help="JSON report generated by analyze."),
    repo: str = typer.Option(..., help="Repository in the format owner/repository."),
    token_env: str = typer.Option("GITHUB_TOKEN", help="Environment variable containing a GitHub token."),
    label: list[str] | None = typer.Option(None, help="GitHub label to attach to the issue. Can be repeated."),
    issue_prefix: str = typer.Option("AWS SRE Doctor", help="Prefix used in the issue title."),
    preview_path: Path = typer.Option(Path("artifacts/diagnosis-github-issue.md"), help="Path for the rendered issue preview."),
    dry_run: bool = typer.Option(False, help="Render the issue preview without calling the GitHub API."),
) -> None:
    report = _load_serialized_file(report_path)
    title = build_issue_title(report, prefix=issue_prefix)
    body = build_issue_body(report)
    preview_path.parent.mkdir(parents=True, exist_ok=True)
    preview_path.write_text(body, encoding="utf-8")

    if dry_run:
        console.print(Panel.fit(f"title={title}\npreview={preview_path.resolve()}", title="GitHub Issue Preview"))
        console.print("Preview gerado sem criar issue remota.")
        return
    ...
    publisher = GitHubIssuePublisher(token=token)
    issue = publisher.create_issue(repo=repo, title=title, body=body, labels=label or [])
    console.print(
        Panel.fit(
